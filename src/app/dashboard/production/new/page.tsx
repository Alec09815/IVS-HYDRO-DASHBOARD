...
import { ArrowLeft, BarChart3, Save, Send } from "lucide-react";
...
export default function NewProductionPage() {
  ...
  useEffect(() => {
    fetchLookups();
    if (employee) setForm((prev) => ({ ...prev, operator_id: employee.id }));
  ...
  const handleSave = async (submit: boolean) => {
    ...
    success(submit ? "Production record submitted" : "Draft saved");
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-ivs-text flex items-center gap-2">
            <BarChart3 size={24} className="text-ivs-accent" /> New Production Entry
          </h1>
      ...
      <div className="bg-ivs-bg-card border border-ivs-border rounded-xl p-5 space-y-4">
        ...
        <textarea value={form.notes} onChange={(e) => update("notes", e.target.value)} rows={3} placeholder="Work performed, conditions, issues encountered..." className={inputCls} />
      </div>

      {/* Actions */}
      <div className="flex items-center justify-end gap-3 pt-2 pb-8">
        <Link href="/dashboard/production" className="px-4 py-2.5 text-sm text-ivs-text-muted hover:text-ivs-text transition-colors">
          Cancel
        </Link>
